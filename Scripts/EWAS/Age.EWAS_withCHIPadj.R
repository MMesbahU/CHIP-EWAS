...
# Load Methylation sample info files with Meth_Age [from Karen] 
...
# DNAm ~ Age_Meth + CHIP + Gender + Race + Batch + 5_cell_types
AGE_ewas_withCHIPadj <- cpg.assoc(CHIP_ordered_methylation_dat, CHIP$agemeth, ~CHIP$haschip + CHIP$Gender + CHIP$RACE + CHIP$Batch_factor + CHIP$CD8T + CHIP$CD4T + CHIP$NK + CHIP$Bcell + CHIP$Mono, chip.id=CHIP$NWD_ID, random=TRUE, large.data = TRUE)

  # Save CHIP ewas results
save(AGE_ewas_withCHIPadj, file = output_filename)
